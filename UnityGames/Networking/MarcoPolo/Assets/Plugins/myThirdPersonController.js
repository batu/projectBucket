...
function Update() {
    ...
    */
	...
	if(_animation) {
		...
		{
		    (GetComponent.<Animation>() as Animation)[attackPoseAnimation.name].AddMixingTransform(buik);
		    (GetComponent.<Animation>() as Animation).CrossFade(attackPoseAnimation.name, 0.2);
		    (GetComponent.<Animation>() as Animation).CrossFadeQueued(idleAnimation.name, 1.0);
		}
	}
	// ANIMATION sector
