...
public function Awake() : void {
	...
	(GetComponent.<Animation>() as Animation)[jumpPoseAnimation.name].wrapMode = WrapMode.ClampForever;
	(GetComponent.<Animation>() as Animation)[fallPoseAnimation.name].wrapMode = WrapMode.ClampForever;
	(GetComponent.<Animation>() as Animation)[idleAnimation.name].wrapMode = WrapMode.Loop;
	(GetComponent.<Animation>() as Animation)[runAnimation.name].wrapMode = WrapMode.Loop;
	(GetComponent.<Animation>() as Animation)[walkAnimation.name].wrapMode = WrapMode.Loop;
}
...
public function Update() : void {
    ...
    //Play (GetComponent.<Animation>() as Animation)
   	if (b_isJumping) {
		if (controller.velocity.y > 0 ) {
			(GetComponent.<Animation>() as Animation)[jumpPoseAnimation.name].speed = jumpAnimationSpeed;
			(GetComponent.<Animation>() as Animation).CrossFade(jumpPoseAnimation.name, 0.1);
		} else {
			(GetComponent.<Animation>() as Animation)[fallPoseAnimation.name].speed = fallAnimationSpeed;
			(GetComponent.<Animation>() as Animation).CrossFade(fallPoseAnimation.name, 0.1);
		}
	} else {
		if (IsAir()) { // Fall down
			(GetComponent.<Animation>() as Animation)[fallPoseAnimation.name].speed = fallAnimationSpeed;
			(GetComponent.<Animation>() as Animation).CrossFade(fallPoseAnimation.name, 0.1);
			 ...
			 //If the character has no velocity or very close to 0 show idle (GetComponent.<Animation>() as Animation)
			if(controller.velocity.sqrMagnitude < 0.1) {
				(GetComponent.<Animation>() as Animation)[idleAnimation.name].speed = idleAnimationSpeed;
				(GetComponent.<Animation>() as Animation).CrossFade(idleAnimation.name, 0.1);
			} else { //Checking if the character walk or run
				if (b_isRun) {
					(GetComponent.<Animation>() as Animation)[runAnimation.name].speed = runAnimationSpeed;
					(GetComponent.<Animation>() as Animation).CrossFade(runAnimation.name, 0.1);
				} else {
					(GetComponent.<Animation>() as Animation)[walkAnimation.name].speed = walkAnimationSpeed;
					(GetComponent.<Animation>() as Animation).CrossFade(walkAnimation.name, 0.1);
				}
			}
		}
	}
